...
void SerialThread::run()
{
    ...
    QByteArray arr = QByteArray::fromHex("4354524C000300030D0A");
    qint64 bytesWritten = serial->write(arr);
    if (bytesWritten == -1) {
        qDebug() << "Failed to write to the port!";
    } else if (bytesWritten != arr.size()) {
        qDebug() << "Not all data written to the port!";
    } else {
